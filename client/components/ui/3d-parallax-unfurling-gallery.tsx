...
/* From 21st.dev — 3-D parallax unfurling gallery. Adapted only so it fills
   its parent (h-full instead of h-screen) and can live inside a slide; the
   visitor scrolls INSIDE it to unfurl the grid. The Unsplash images are the
   component's own placeholders — swap for real photography before production. */
...
interface ImageCardProps { src: string; onLoad?: () => void }
...
const ImageCard = ({ src, onLoad }: ImageCardProps) => (
  <div className="w-full h-[200px] sm:h-[300px] md:h-[400px] flex-shrink-0 bg-[#111] transition-transform duration-300 hover:scale-[1.02] cursor-pointer relative will-change-transform backface-hidden preserve-3d">
    <img src={src} alt="Gallery Asset" loading="lazy" onLoad={onLoad} className="w-full h-full object-cover opacity-80 hover:opacity-100 transition-opacity duration-300" />
  </div>
);
...
export default function Component() {
  ...
  const colMedia = useMemo(() => {
    ...
    return { col1: [...col1Base, ...col1Base], col2: [...col2Base, ...col2Base], col3: [...col3Base, ...col3Base], col4: [...col4Base, ...col4Base] };
  }, []);
  ...
  const bannerWidth = useTransform(smoothProgress, [0, 0.15], ["90%", "100%"]);
  const bannerHeight = useTransform(smoothProgress, [0, 0.15], ["80%", "100%"]);
  ...
  return (
    <div ref={scrollWrapperRef} className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#050505]">
      <section ref={containerRef} className="relative w-full h-[600%] bg-[#050505] text-white font-sans selection:bg-white selection:text-black">
        <div className="sticky top-0 h-[16.6667%] w-full flex justify-center items-center overflow-hidden">
          <motion.div
            style={{ width: bannerWidth, height: bannerHeight, borderRadius: bannerRadius, borderWidth: bannerBorderWidth, borderColor: "#2c2738" }}
            ...
            <div className="absolute inset-0 flex justify-center items-center pointer-events-none" style={{ perspective: "1000px" }}>
              ...
              >
                <motion.div style={{ y: yCol1 }} className="flex flex-col gap-4 md:gap-6 w-[22vw] min-w-[200px] pointer-events-auto">
                  {colMedia.col1.map((src, index) => <ImageCard key={`col1-${index}`} src={src} onLoad={handleItemLoad} />)}
                </motion.div>
                <motion.div style={{ y: yCol2 }} className="flex flex-col gap-4 md:gap-6 w-[22vw] min-w-[200px] pointer-events-auto">
                  {colMedia.col2.map((src, index) => <ImageCard key={`col2-${index}`} src={src} onLoad={handleItemLoad} />)}
                </motion.div>
                <motion.div style={{ y: yCol3 }} className="flex flex-col gap-4 md:gap-6 w-[22vw] min-w-[200px] pointer-events-auto">
                  {colMedia.col3.map((src, index) => <ImageCard key={`col3-${index}`} src={src} onLoad={handleItemLoad} />)}
                </motion.div>
                <motion.div style={{ y: yCol4 }} className="flex flex-col gap-4 md:gap-6 w-[22vw] min-w-[200px] pointer-events-auto">
                  {colMedia.col4.map((src, index) => <ImageCard key={`col4-${index}`} src={src} onLoad={handleItemLoad} />)}
                </motion.div>
              </motion.div>
            </div>
          </motion.div>
